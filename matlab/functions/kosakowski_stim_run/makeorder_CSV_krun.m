function makeorder_CSV_krun(participant, run)
...
csvfile1 = fullfile(participant_folder, strcat('kosakowski_blocks_run', num2str(run), '.csv')); 
...
blockNumbers = data1{:, 1};  % Assuming the first column contains the block numbers

% Identify unique block numbers
uniqueBlocks = unique(blockNumbers);

BlockRemap = [2, 4, 6, 8, 10, 12, 14, 16];


% Shuffle each array
shuffledBlocks1 = shuffle([2, 4, 6, 8]);
shuffledBlocks2 = shuffle([10, 12, 14, 16]);


uniqueBlocks = [shuffledBlocks1, shuffledBlocks2];
...
for i = 1:length(uniqueBlocks)
    ...
    groupedData{i} = data1(blockIndices, :);
end
